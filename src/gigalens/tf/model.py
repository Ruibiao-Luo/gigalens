...
class ForwardProbModel(gigalens.model.ProbabilisticModel):
    ...
    *are not* automatically solved for using least squares.
    ...
    Attributes:
        ...
        mask_image (:obj:`tf.Tensor` or :obj:`numpy.array`): The mask of observed image.
    ...
    def log_prob(self, simulator: gigalens.tf.simulator.LensSimulator, z):
        ...
        err_map_valid = tf.math.sqrt(self.background_rms ** 2 + tf.clip_by_value(im_sim, 0, np.inf) / self.exp_time)
        err_map = tf.where(self.mask_image > 0, err_map_valid, large_error)
...
class BackwardProbModel(gigalens.model.ProbabilisticModel):
    ...
    *are* automatically solved for using least squares.
    ...
    def __init__(
        ...
        self.err_map_valid = tf.math.sqrt(
            background_rms ** 2 + 
            tf.clip_by_value(masked_observed, 0, np.inf) / exp_time
        )
        self.err_map = tf.where(self.mask_image > 0, self.err_map_valid, large_error)
